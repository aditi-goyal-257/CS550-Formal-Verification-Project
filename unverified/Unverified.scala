...
object ClosestPointUnverified:
    ...
    def mergeX(l1: List[Point], l2: List[Point]): List[Point]={
        ...
        def mergeXAcc(l1: List[Point], l2: List[Point] , acc: List[Point] = List[Point]() ): List[Point] = {
            if(l1.isEmpty && l2.isEmpty) then acc else {
                if(l1.isEmpty) then mergeXAcc(l1 , l2.tail , l2.head :: acc) else {
                    if(l2.isEmpty) then mergeXAcc( l1.tail, l2,  l1.head :: acc) else {
                        if(l1.head.x <= l2.head.x) then mergeXAcc(l1.tail , l2 , l1.head :: acc) else mergeXAcc(l1 , l2.tail , l2.head :: acc)
                    }
                }
            }
    ...
    def mergeY(l1: List[Point], l2: List[Point]): List[Point]={
        ...
        @scala.annotation.tailrec
        def mergeYAcc(l1: List[Point], l2: List[Point] , acc: List[Point] = List[Point]() ): List[Point] = {
            if(l1.isEmpty && l2.isEmpty) then acc else {
                if(l1.isEmpty) then mergeYAcc(l1 , l2.tail , l2.head :: acc) else {
                    if(l2.isEmpty) then mergeYAcc( l1.tail, l2,  l1.head :: acc) else {
                        if(l1.head.y <= l2.head.y) then mergeYAcc(l1.tail , l2 , l1.head :: acc) else mergeYAcc(l1 , l2.tail , l2.head :: acc)
                    }
                }
            }
